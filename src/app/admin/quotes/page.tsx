                            ...
                            <div className="p-8 md:p-12 space-y-8">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden group">
                                        ...
                                        <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mb-2 block italic">Información del Cliente</p>
                                        <div className="space-y-1.5">
                                            <div className="flex flex-col">
                                                <span className="text-sm font-black tracking-tight">{selectedQuote.customer_name || 'No proporcionado'}</span>
                                                <span className="text-[10px] font-medium text-slate-400">{selectedQuote.customer_email || 'Sin correo electrónico'}</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-[11px] font-bold py-1 border-y border-slate-50 dark:border-slate-800">
                                                <div className="h-5 w-5 rounded-md bg-primary/10 flex items-center justify-center text-primary italic">@</div>
                                                {selectedQuote.customer_phone || 'Sin WhatsApp'}
                                            </div>
                                            <div className="text-[9px] font-semibold leading-relaxed text-slate-500 italic">
                                                {selectedQuote.shipping_address || 'Sin dirección de envío'}
                                            </div>
                                        </div>
                                    </div>

                                    <div className="bg-amber-500 p-4 rounded-2xl shadow-xl shadow-amber-500/20 flex flex-col justify-between relative overflow-hidden group text-white">
                                        <div className="absolute top-0 right-0 p-3 opacity-20 group-hover:rotate-12 transition-transform duration-500 pointer-events-none">
                                            <TrendingUp size={48} />
                                        </div>
                                        <div className="relative z-10">
                                            <p className="text-[9px] font-black uppercase tracking-widest text-white/80 mb-2 block italic">Resumen de Cotización</p>
                                        </div>
                                        <div className="space-y-0 relative z-10 text-right mt-auto">
                                            <span className="text-[9px] font-black uppercase tracking-widest opacity-80">Total a Pagar</span>
                                            <h2 className="text-2xl lg:text-3xl font-black italic tracking-tighter leading-none">${selectedQuote.total.toFixed(2)}</h2>
                                            {settings?.exchange_rate && (
                                                <p className="text-[9px] font-bold opacity-90 italic mt-0.5">≈ Bs {(selectedQuote.total * settings.exchange_rate).toLocaleString('es-VE', { minimumFractionDigits: 2 })}</p>
                                            )}
                                        </div>
                                    </div>
                                </div>
