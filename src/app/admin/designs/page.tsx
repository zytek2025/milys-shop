...
export default function AdminDesignsPage() {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Global Pricing Summary Section */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <Card className="lg:col-span-2 border-none shadow-sm bg-slate-50/50 dark:bg-slate-900/50 overflow-hidden">
                    <CardHeader className="flex flex-row items-center justify-between pb-4">
                        <div className="space-y-1">
                            <h3 className="text-sm font-black uppercase italic tracking-tighter flex items-center gap-2">
                                <Palette className="text-primary w-4 h-4" /> Precios Globales de Logos / Diseños
                            </h3>
                            <p className="text-[10px] text-muted-foreground font-bold uppercase">Estos precios se aplican a todos los diseños de la biblioteca.</p>
                        </div>
                        <Button
                            size="sm"
                            disabled={isSavingSettings}
                            onClick={handleSaveSettings}
                            className="rounded-lg h-8 px-4 font-black italic text-[10px] uppercase shadow-md shadow-primary/10"
                        >
                            {isSavingSettings ? <Loader2 className="animate-spin w-3 h-3" /> : <Save className="w-3 h-3 mr-1.5" />}
                            Actualizar Precios
                        </Button>
                    </CardHeader>
                    <CardContent className="grid grid-cols-3 gap-4 pb-6">
                        <div className="space-y-1.5">
                            <Label className="text-[9px] uppercase font-black text-blue-500">Logo Pequeño (S)</Label>
                            <Input
                                type="number"
                                step="0.01"
                                className="h-10 rounded-lg border-2 bg-white dark:bg-slate-950 font-bold px-3"
                                value={storeSettings.design_price_small}
                                onChange={e => updateSettingField('design_price_small', e.target.value)}
                            />
                        </div>
                        <div className="space-y-1.5">
                            <Label className="text-[9px] uppercase font-black text-purple-500">Logo Mediano (M)</Label>
                            <Input
                                type="number"
                                step="0.01"
                                className="h-10 rounded-lg border-2 bg-white dark:bg-slate-950 font-bold px-3"
                                value={storeSettings.design_price_medium}
                                onChange={e => updateSettingField('design_price_medium', e.target.value)}
                            />
                        </div>
                        <div className="space-y-1.5">
                            <Label className="text-[9px] uppercase font-black text-orange-500">Logo Grande (L)</Label>
                            <Input
                                type="number"
                                step="0.01"
                                className="h-10 rounded-lg border-2 bg-white dark:bg-slate-950 font-bold px-3"
                                value={storeSettings.design_price_large}
                                onChange={e => updateSettingField('design_price_large', e.target.value)}
                            />
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="border-none shadow-sm bg-slate-50/50 dark:bg-slate-900/50 overflow-hidden">
                    <CardHeader className="pb-4">
                        <h3 className="text-sm font-black uppercase italic tracking-tighter flex items-center gap-2">
                            <Type className="text-primary w-4 h-4" /> Personalización de Texto
                        </h3>
                    </CardHeader>
                    <CardContent className="grid grid-cols-2 gap-4 pb-6">
                        <div className="space-y-1.5">
                            <Label className="text-[9px] uppercase font-black text-primary">Texto Pequeño</Label>
                            <Input
                                type="number"
                                step="0.01"
                                className="h-10 rounded-lg border-2 bg-white dark:bg-slate-950 font-bold px-3"
                                value={storeSettings.personalization_price_small}
                                onChange={e => updateSettingField('personalization_price_small', e.target.value)}
                            />
                        </div>
                        <div className="space-y-1.5">
                            <Label className="text-[9px] uppercase font-black text-primary">Texto Grande</Label>
                            <Input
                                type="number"
                                step="0.01"
                                className="h-10 rounded-lg border-2 bg-white dark:bg-slate-950 font-bold px-3"
                                value={storeSettings.personalization_price_large}
                                onChange={e => updateSettingField('personalization_price_large', e.target.value)}
                            />
                        </div>
                    </CardContent>
                </Card>
